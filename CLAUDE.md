...
- ✅ **دليل استخدام البرنامج:** `دليل-استخدام-البرنامج.html` بجذر المشروع — ملف HTML واحد قائم بذاته (صور شاشات
  حقيقية مُضمَّنة base64، بيانات تجريبية فقط) يشرح كل شاشة: الغرض منها وخطوات استخدامها، موجَّه للمستخدم غير
  التقني. يُفتح بأي متصفح مباشرة؛ لنسخة PDF/ورقية: طباعة من المتصفح (Ctrl+P) ← حفظ كـPDF. حُدِّث آخر مرة بعد
  ميزة "بحث عن عائلة + صفحة تفاصيل عائلة" — **يحتاج تحديث يدوي عند أي تغيير على شاشات الواجهة مستقبلاً** (لا يتم
  تلقائياً).
